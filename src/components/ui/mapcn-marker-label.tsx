...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    // Dubai map bounds — prevent dragging into empty ocean
    const dubaiBounds = new MapLibreGL.LngLatBounds(
      [54.8, 24.8], // SW corner
      [55.6, 25.4]  // NE corner
    );
...
function MarkerLabel({ children, className, position = "top" }: MarkerLabelProps) {
  ...
  return (
    <div className={cn("absolute left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] font-medium text-foreground shadow-sm", positionClasses[position], className)}>
      {children}
    </div>
  );
}
